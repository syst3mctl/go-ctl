...
{{if eq .DbDriver.ID "gorm"}}// New creates a new PostgreSQL repository instance using GORM
...
{{else if eq .DbDriver.ID "sqlx"}}// New creates a new PostgreSQL repository instance using sqlx
func New(db *sqlx.DB) *Repository {
	return &Repository{db: db}
}

// Migrate runs database migrations for PostgreSQL using sqlx
func (r *Repository) Migrate() error {
	// Create users table
	userTable := `
...
func (r *Repository) CreateUser(ctx context.Context, user *domain.User) error {
	...
	now := time.Now()
	err := r.db.QueryRowContext(ctx, query, user.Name, user.Email, user.Active, now, now).
...
// DeleteUser deletes a user from PostgreSQL (hard delete for sqlx)
...
// Transaction wraps operations in a PostgreSQL transaction using sqlx
