...
// Repository implements SQLite-specific repository operations
...
{{if eq .DbDriver.ID "gorm"}}// New creates a new SQLite repository instance using GORM
func New(db *gorm.DB) *Repository {
	return &Repository{db: db}
}

// Migrate runs database migrations for SQLite using GORM
...
// GetUserByID retrieves a user by ID from SQLite
...
// GetUserByEmail retrieves a user by email from SQLite
...
// DeleteUser soft deletes a user in SQLite
...
// ListUsers retrieves users with pagination from SQLite
...
// CountUsers returns the total number of users in SQLite
...
// SearchUsers searches users by name or email in SQLite
...
// CreateProduct creates a new product in SQLite
...
// GetProductByID retrieves a product by ID from SQLite
...
// ListProducts retrieves products with pagination from SQLite
...
// Transaction wraps operations in a SQLite transaction using GORM
func (r *Repository) Transaction(ctx context.Context, fn func(*gorm.DB) error) error {
	return r.db.WithContext(ctx).Transaction(fn)
}

{{else if eq .DbDriver.ID "sqlx"}}// New creates a new SQLite repository instance using sqlx
func New(db *sqlx.DB) *Repository {
	return &Repository{db: db}
}

// Migrate runs database migrations for SQLite using sqlx
func (r *Repository) Migrate() error {
	// Create users table
	userTable := `
...
// DeleteUser deletes a user from SQLite (hard delete for sqlx)
...
// Transaction wraps operations in a SQLite transaction using sqlx
